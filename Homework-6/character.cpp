...
uint32_t can_see(map_t *m, npc *voyeur, pc *exhibitionist)
{
...
static void move_hiker_func(character *c, pair_t dest)
{
  ...
  for (i = base; i < 8 + base; i++)
  {
    if ((world.hiker_dist[n->pos[dim_y] + all_dirs[i & 0x7][dim_y]]
                         [n->pos[dim_x] + all_dirs[i & 0x7][dim_x]] <=
         min) &&
        !world.cur_map->cmap[n->pos[dim_y] + all_dirs[i & 0x7][dim_y]]
                            [n->pos[dim_x] + all_dirs[i & 0x7][dim_x]] &&
        n->pos[dim_x] + all_dirs[i & 0x7][dim_x] != 0 &&
        n->pos[dim_x] + all_dirs[i & 0x7][dim_x] != MAP_X - 1 &&
        n->pos[dim_y] + all_dirs[i & 0x7][dim_y] != 0 &&
        n->pos[dim_y] + all_dirs[i & 0x7][dim_y] != MAP_Y - 1)
    {
      dest[dim_x] = n->pos[dim_x] + all_dirs[i & 0x7][dim_x];
      dest[dim_y] = n->pos[dim_y] + all_dirs[i & 0x7][dim_y];
...
static void move_wanderer_func(character *c, pair_t dest)
{
  ...
  if ((world.cur_map->map[n->pos[dim_y] + n->dir[dim_y]]
                         [n->pos[dim_x] + n->dir[dim_x]] !=
       world.cur_map->map[n->pos[dim_y]][c->pos[dim_x]]) ||
      world.cur_map->cmap[n->pos[dim_y] + n->dir[dim_y]]
                         [n->pos[dim_x] + n->dir[dim_x]])
  {
    rand_dir(n->dir);
...
static void move_explorer_func(character *c, pair_t dest)
{
  ...
  if ((move_cost[char_other][world.cur_map->map[n->pos[dim_y] +
                                               ...
                                               [n->pos[dim_x] +
                                                n->dir[dim_x]]] ==
       DIJKSTRA_PATH_MAX) ||
      (world.cur_map->cmap[n->pos[dim_y] +
                           n->dir[dim_y]]
